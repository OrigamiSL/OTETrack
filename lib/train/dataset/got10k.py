...
class Got10k(BaseVideoDataset):
    ...
    def _get_sequence_path(self, seq_id):
        list_file = os.listdir(self.root)
        list_file.remove('list.txt')
        true_name = ''
        if self.root == '/home/lhg/work/fxy/TCTrack-main-test/train_dataset/got10k/data/val':
            return os.path.join(self.root, self.sequence_list[seq_id])
        else :
            for i in list_file:
                x_name = os.listdir(os.path.join(self.root, i))
                if self.sequence_list[seq_id] in x_name:
                    true_name = os.path.join(i,self.sequence_list[seq_id])
                    break
        # return os.path.join(self.root, self.sequence_list[seq_id])
        
            return os.path.join(self.root, true_name)
    ...
    def get_frames(self, seq_id, frame_ids, anno=None,get_last_frame = False):
        # print('strat_get_frame')
        seq_path = self._get_sequence_path(seq_id)
        obj_meta = self.sequence_meta_info[self.sequence_list[seq_id]]
        if not get_last_frame:
            frame_list = [self._get_frame(seq_path, f_id) for f_id in frame_ids]
        else:
            f_id = frame_ids[-1]
            frame_list = [self._get_frame(seq_path, f_id)]
        ...
        # print(frame_list, anno_frames, obj_meta)
