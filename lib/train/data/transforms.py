...
class TransformBase:
    ...
    def __call__(self, **inputs):
        ...
        outputs = dict()
        # print('len',len(input_vars.items()))
        if len(input_vars.items()) == 2: 
...
class RandomHorizontalFlip(TransformBase):
    ...
    def __init__(self, probability = 0.5):
        super().__init__()
        self.probability = probability
        # self.RandomHorizontalFlip = False
    def roll(self):
        # if random.random() < self.probability:
        #     self.RandomHorizontalFlip = True
        # print('self.RandomHorizontalFlip',self.RandomHorizontalFlip)
        return random.random() < self.probability
