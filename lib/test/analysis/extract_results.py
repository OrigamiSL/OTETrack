...
def extract_results(trackers, dataset, report_name, skip_missing_seq=False, plot_bin_gap=0.05,
    ...
    for seq_id, seq in enumerate(tqdm(dataset)):
        ...
        for trk_id, trk in enumerate(trackers):
            ...
            # base_results_path = '{}/{}'.format('/home/lhg/work/fxy_ar1/AR_update/lasot_test_result_ext/OTETrack_ep0150.pth.tar', seq.name)
            # base_results_path = '{}/{}'.format('/home/lhg/work/fxy_ar1/AR_update/lasot_test_result/OTETrack_ep0025.pth.tar', seq.name)
            
            results_path = '{}.txt'.format(base_results_path)
            # if trk_id == 0:
            #     x_first_frame = os.path.join(trk.results_dir, 'got10k')
            #     base_results_path = '{}/{}'.format(x_first_frame, seq.name)
            # else:
            #     base_results_path = '{}/{}'.format(trk.results_dir, seq.name)
            # results_path = '{}.txt'.format(base_results_path)
